...
@CapabilityDescription("Format and send Records to a configured uri using HTTP post. The Record Writer formats the records which are sent as the body of the http post request. " +
        "JsonRecordSetWriter is a commonly used Record Writer because many https posts use JSON format.")
public class HttpRecordSink extends AbstractControllerService implements RecordSinkService {
    ...
    static final PropertyDescriptor MAX_BATCH_SIZE = new PropertyDescriptor.Builder()
            ...
            .description("Specifies maximum number of records to sent in the body of each http request. Zero means the batch size is not limited, "
    ...
    public static final PropertyDescriptor WEB_SERVICE_CLIENT_PROVIDER = new PropertyDescriptor.Builder()
            ...
            .description("Controller service to provide HTTP client for sending HTTP requests")
            .required(true)
    ...
    protected PropertyDescriptor getSupportedDynamicPropertyDescriptor(final String propertyDescriptorName) {
        if (hasCaseMismatch(propertyDescriptorName, HEADER_CONTENT_TYPE)) {
        ...
        if (hasCaseMismatch(propertyDescriptorName, HEADER_AUTHORIZATION)) {
    ...
    private static boolean hasCaseMismatch(String userInput, String correctName) {
        // Do not allow : in any case because it is not the correct name
